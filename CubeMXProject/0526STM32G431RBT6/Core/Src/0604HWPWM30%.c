 ...
 *
 * 風扇來回週期：30000ms（0→100→0）
 *   每 150ms 更新一次，共 200步
 *   0→100：100步 × 150ms = 15000ms
 *   100→0：100步 × 150ms = 15000ms
 *
 * LED 來回週期：15000ms（0→100→0）
 *   每 75ms 更新一次，共 200步
 *   0→100：100步 × 75ms = 7500ms
 *   100→0：100步 × 75ms = 7500ms
 *
 * PWM 頻率：1kHz，ARR=999
...
/* ── PWM 參數 ── 往後想調整速度只需改這兩行：150U跟75U */
#define PWM_ARR               999U  /* 1kHz */
#define MOTOR_UPDATE_INTERVAL 150U  /* ms，風扇每步間隔（30秒來回）*/
#define LED_UPDATE_INTERVAL    75U  /* ms，LED  每步間隔（15秒來回）*/

/* 馬達啟動門檻：內部 pct 0~100 映射到實際 MIN~100
 * 停止時（running=0）才真正輸出 0%（全 HIGH）
 * 循環中最低輸出 MOTOR_MIN_PCT，保持馬達持續轉動 */
//如果 30% 還不夠讓馬達轉，只需調這一行：
#define MOTOR_MIN_PCT          30U  /* 調高這個數字，例如改成 40 或 50，可調：實際最低輸出 % */
...
/* 低態驅動 pct → CCR（馬達用，0~100 映射到 MIN~100）
 * pct=0   → 實際輸出 MOTOR_MIN_PCT
 * pct=100 → 實際輸出 100%
 * running=0 時直接給 PWM_ARR（全停）                */
static inline uint32_t motor_pct_to_ccr(uint8_t pct)
{
    /* 映射：actual = MIN + pct * (100 - MIN) / 100 */
...
int main(void)
{
    ...
    for (;;)
    {
            ...
            if ((now - btnM.debounce) >= 300UL)
            {
                if (!motor.running)
                {
                    ...
                    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1,
                                          motor_pct_to_ccr(0)); /* 實際輸出 MIN_PCT */
                }
                else
                {
                    /* 停止：真正全停（全 HIGH）*/
        ...
        /* ── 馬達來回更新（每 150ms 走 1%，30秒來回）── */
        if (motor.running && (now - motor.last_tick) >= MOTOR_UPDATE_INTERVAL)
        {
            motor.last_tick = now;
            ...
            __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, motor_pct_to_ccr(motor.pct));
        }

        /* ── LED 來回更新（每 75ms 走 1%，15秒來回）── */
        if (led.running && (now - led.last_tick) >= LED_UPDATE_INTERVAL)
        {
            led.last_tick = now;
